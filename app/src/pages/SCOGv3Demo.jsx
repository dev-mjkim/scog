...
import { CREDENTIALS_V3, SCOG_V3_URL, COG_URL, EXTENT } from '../credentials.js';
...
export default function SCOGv3Demo() {
  ...
  // ── SCOG v3 로드 ─────────────────────────────────────────────────────────────
  const loadSCOGv3 = useCallback(async () => {
    if (loading) return;
    setLoading(true);
    setReqLog([]);
    setStats({ requests: 0, bytesTotal: 0, timeMs: 0 });
    ...
    const cred   = CREDENTIALS_V3[tier];
    const client = new SCOGClient(SCOG_V3_URL, cred, {
      onRequest: (info) => {
    ...
    // v3: 16KB 1회 요청으로 IFD + SCOG 블록 파싱
    await client.loadScogBlock();

    const levels     = client.availableLevels();
    const encLevels  = Object.keys(client.cred.levels).map(Number);
    const isPublic   = (lvl) => !encLevels.includes(lvl);
    const blocked    = encLevels.filter(l => !levels.includes(l));

    try {
      const renderLevel = Math.min(...levels);
      const meta        = client.levelMeta(renderLevel);
      const isPublicLevel = isPublic(renderLevel);
  ...
  // v3: 공개 레벨(L2)은 자격증명 없이 접근 가능
  const accessible = [...new Set([...tierEncLevels, 2])];
      ...
      <div className="main">
        ...
        <div className="sidebar">
          ...
          <div className="panel" style={{ background: '#1a2035', borderColor: '#2d4a7a' }}>
            <div className="panel-title" style={{ color: '#b794f4' }}>⚡ SCOG v3 (1-Request 메타데이터)</div>
            <div style={{ fontSize: '11px', color: '#718096', lineHeight: 1.6 }}>
              파일 구조: <code style={{ color: '#b794f4' }}>[TIFF IFDs][SCOG Block][타일 데이터][새 메인 IFD]</code><br/>
              • <strong style={{ color: '#b794f4' }}>SCOG 블록이 첫 16KB 내</strong> → 1회 요청으로 IFD + SCOG 획득<br/>
              • QGIS 호환 유지 (v2와 동일)<br/>
          ...
          <div className="panel">
            ...
            <button className="load-btn scog" onClick={loadSCOGv3} disabled={loading}>
              {loading ? '로드 중...' : '▶ Load SCOG v3 Image'}
            </button>
          ...
          <div className="panel">
            ...
            {[
              { id: 0, title: 'Full Resolution', sub: '1305×1107 · 9 타일 · 🔐 암호화 (L0 CEK → Admin)', pub: false },
              { id: 1, title: 'Overview × ½',    sub: '652×553 · 4 타일 · 🔐 암호화 (L1 CEK → User+)', pub: false },
              { id: 2, title: 'Overview × ¼',    sub: '326×276 · 1 타일 · 🌐 공개 (암호화 없음)',       pub: true },
            ].map(({ id, title, sub, pub }) => {
              const isPublicId = pub;
