...
def cog_to_scog(input_path: str, output_path: str,
                ...
                public_level_arg: int = None) -> dict:
    """
    COG 파일을 SCOG로 변환.
    format_ver: 'v1' (SCOG prefix) or 'v2' (TIFF-first, QGIS 호환) or 'v3' (SCOG-in-header, 1-request)
...
예시:
  ...
  python src/cog_to_scog.py data/kompsat.tif data/kompsat.scog --keys keys/
    ...
    parser.add_argument('--format', dest='format_ver', choices=['v1', 'v2', 'v3'], default='v1',
                        help='SCOG 포맷 버전 (v1=SCOG prefix, v2=TIFF-first QGIS 호환, v3=SCOG-in-header 1-request)')
    ...
    cog_to_scog(args.input, args.output, keys_dir, args.format_ver, args.public_level)
